    ...
    execute {
        Fingerprint(
            ...
            for (instr in instructions) {
                if (instr.opcode.name == "new-instance" && instr is com.android.tools.smali.dexlib2.iface.instruction.OneRegisterInstruction) {
                    listReg = instr.registerA
                    break
                }
            }
            if (listReg == -1) return@let

            for (i in instructions.indices.reversed()) {
                val instr = instructions[i]
                if (instr.opcode.name == "return-object" && instr is com.android.tools.smali.dexlib2.iface.instruction.OneRegisterInstruction) {
                    if (instr.registerA == listReg) {
                        val tempReg = if (listReg == 0) 1 else 0
                        match.method.addInstructions(i, """
